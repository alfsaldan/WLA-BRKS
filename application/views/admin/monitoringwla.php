...
<!-- Filter Form -->
<form method="GET" action="<?= site_url('admin/monitoring') ?>" class="card glass p-3 mb-4 border-0 shadow-sm d-print-none">
...
<?php if($filter_bulan === 'all'): ?>
    <ul class="nav nav-tabs mb-3 d-print-none" id="laporanTabs" role="tablist">
      <li class="nav-item" role="presentation">
        <button class="nav-link active fw-bold" id="tab-pegawai" data-bs-toggle="tab" data-bs-target="#pane-pegawai" type="button" role="tab"><i class="bi bi-people me-2"></i>Rekap per Pegawai</button>
      </li>
      <li class="nav-item" role="presentation">
        <button class="nav-link fw-bold" id="tab-bulan" data-bs-toggle="tab" data-bs-target="#pane-bulan" type="button" role="tab"><i class="bi bi-calendar3 me-2"></i>Rekap per Bulan (Jan-Des)</button>
      </li>
    </ul>
<?php endif; ?>
...
<div class="tab-pane fade show active" id="pane-pegawai" role="tabpanel">
    ...
    <div class="mb-4">
        <div class="d-flex justify-content-between align-items-center mb-4">
            <h4 class="fw-bold mb-0" style="letter-spacing: 2px;">WORK LOAD ANALYSIS</h4>
...
<?php if($filter_bulan === 'all'): ?>
<div class="tab-pane fade" id="pane-bulan" role="tabpanel">
    <div class="card bg-white p-4 shadow-sm border-0 mb-5" style="border-radius: 0; overflow-x: auto;">
        <!-- Laporan Header -->
        <div class="mb-4">
            <div class="d-flex justify-content-between align-items-center mb-4">
                <h4 class="fw-bold mb-0" style="letter-spacing: 2px;">WORK LOAD ANALYSIS (JAN-DES)</h4>
                <button class="btn btn-sm btn-outline-success d-print-none" onclick="window.print()"><i class="bi bi-printer me-2"></i>Cetak Laporan</button>
            </div>
            <table class="table table-sm table-borderless fw-bold" style="width: auto;">
                <tr><td width="30">1</td><td width="200">NAMA JABATAN</td><td width="10">:</td><td><?= strtoupper($nama_jab) ?></td></tr>
        ...
        <table class="table table-bordered align-middle" style="font-size: 0.85rem; border-color: #dee2e6; min-width: 1400px;">
            <thead class="text-center align-middle bg-light">
                <tr>
                    <th rowspan="2" width="40">NO</th>
                    <th rowspan="2" style="min-width: 250px;">URAIAN TUGAS & TANGGUNG JAWAB</th>
                    <th rowspan="2" style="min-width: 150px;">HASIL KERJA / OUTPUT</th>
            ...
            <tbody>
                <?php if(empty($tugas)): ?>
                    <tr><td colspan="19" class="text-center text-muted py-3">Tidak ada data uraian tugas.</td></tr>
                <?php endif; ?>
                
                <?php 
                $no = 1;
                $total_all_standar_waktu_m = 0;
                $total_all_rpm_m = 0;
                ...
                foreach($parents as $t): 
                    ...
                    if (!$is_group) {
                        for($i=1; $i<=12; $i++) {
                            $vol = isset($monthly_volumes_jabatan[$t->id_tugas][$i]) ? $monthly_volumes_jabatan[$t->id_tugas][$i] : 0;
                            $total_volume += $vol; 
                        }
                    }
                    ...
                    if (!$is_group) {
                        $total_all_standar_waktu_m += (float)$t->standar_waktu;
                        $total_all_rpm_m += $rpm;
                    }
                ?>
                <tr <?= $is_group ? 'class="bg-light fw-bold"' : '' ?>>
                    ...
                    <?php for($i=1; $i<=12; $i++): 
                        $vol_val = isset($monthly_volumes_jabatan[$t->id_tugas][$i]) && $monthly_volumes_jabatan[$t->id_tugas][$i] > 0 ? $monthly_volumes_jabatan[$t->id_tugas][$i] : '-';
                    ?>
                ...
                foreach($children as $c): if($c->id_parent == $t->id_tugas): 
                    ...
                    $c_total_volume = 0;
                    for($i=1; $i<=12; $i++) {
                        $c_vol = isset($monthly_volumes_jabatan[$c->id_tugas][$i]) ? $monthly_volumes_jabatan[$c->id_tugas][$i] : 0;
                        $c_total_volume += $c_vol; 
                    }
                    
                    $c_avg = $c_total_volume / ($emp_count * 12);
                    $c_rata_rata_transaksi = ceil($c_avg);
                    
                    $c_rpm = ($c->standar_waktu !== null) ? ($c_rata_rata_transaksi * $c->standar_waktu) : 0;
                    
                    $total_all_standar_waktu_m += (float)$c->standar_waktu;
                    $total_all_rpm_m += $c_rpm;
                ?>
                <tr>
                    <td class="text-center"><?= $char++ ?></td>
                    <td style="white-space: normal; padding-left: 2rem;"><?= nl2br(htmlspecialchars($c->nama_tugas ?? '')) ?></td>
                    ...
                    <?php for($i=1; $i<=12; $i++): 
                        $c_vol_val = isset($monthly_volumes_jabatan[$c->id_tugas][$i]) && $monthly_volumes_jabatan[$c->id_tugas][$i] > 0 ? $monthly_volumes_jabatan[$c->id_tugas][$i] : '-';
                    ?>
            ...
            <tfoot class="fw-bold bg-light">
                <?php 
                    $allowance_val = ($allowance_pct / 100) * $total_all_rpm_m;
                    $total_rpm_plus_allowance = $total_all_rpm_m + $allowance_val;
                    
                    $fte_m = $total_rpm_plus_allowance > 0 ? ($total_rpm_plus_allowance / $wke_default) : 0;
                    $kebutuhan_m = round($fte_m);
                ?>
                <tr>
                    <td colspan="15" class="text-end">JUMLAH</td>
                    <td class="text-center"><?= $total_all_standar_waktu_m > 0 ? $total_all_standar_waktu_m : '-' ?></td>
                    <td class="text-center"><?= $total_all_rpm_m > 0 ? $total_all_rpm_m : '-' ?></td>
                    <td></td>
                </tr>
                <tr>
                    <td colspan="15" class="text-end text-primary align-middle">
                        JUMLAH KEBUTUHAN PEGAWAI (FTE)<br>
                        <span class="text-muted fw-normal" style="font-size:0.7rem;">Rumus: (Total RPM + <?= $allowance_pct ?>%) / <?= $wke_default ?></span>
                    </td>
                    <td colspan="3" class="text-start text-primary h6 mb-0 py-3 align-middle">
                        <?= number_format($fte_m, 2, ',', '.') ?> &nbsp; <i class="bi bi-arrow-right"></i> &nbsp; Pembulatan: <?= $kebutuhan_m ?>
                    </td>
                </tr>
            </tfoot>
        </table>
